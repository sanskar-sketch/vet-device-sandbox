...
// Brand tokens are kept separate from NAVY (the body-text colour) so the
// wordmark stays true to the logo without restyling every heading.
const BRAND_NAVY = '#123a5c', BRAND_TEAL = '#2bb5a6', BRAND_SUB = '#5c7b96';
...
function buildReportPdf(exam, pet, photo) {
  return new Promise((resolve, reject) => {
    ...
    // ── Header ──────────────────────────────────────────────────────────
    // The wordmark is drawn as text, not an embedded logo image: it's a
    // typographic mark, so Helvetica-Bold with tracking reproduces it, and
    // this avoids shipping a raster asset that would soften when printed.
    //
    // The teal rule sits under the "A", so its x/width are measured off the
    // glyphs rather than hard-coded. pdfkit's characterSpacing is emitted as
    // the PDF Tc operator, which adds spacing after *every* glyph, while
    // widthOfString() only counts the (n-1) internal gaps — hence the extra
    // TRACKING added to the "VIT" offset. Without that, the rule drifts left.
    const TRACKING = 1.2, brandTop = 50;
    doc.fillColor(BRAND_NAVY).font('Helvetica-Bold').fontSize(22);
    doc.text('VITARUS', 50, brandTop, { characterSpacing: TRACKING, lineBreak: false });
    const ruleX = 50 + doc.widthOfString('VIT', { characterSpacing: TRACKING }) + TRACKING;
    const ruleW = doc.widthOfString('A', { characterSpacing: TRACKING }) + TRACKING;
    doc.rect(ruleX, brandTop + 25, ruleW, 2.8).fill(BRAND_TEAL);

    doc.fillColor(BRAND_SUB).font('Helvetica').fontSize(7.5)
      .text('ANIMAL DIAGNOSTICS', 50, brandTop + 34, { characterSpacing: 2.4, lineBreak: false });
    doc.fillColor(MUTED).font('Helvetica').fontSize(10)
      .text('Multi-Modal Veterinary Diagnostic Report', 300, brandTop + 10, { width: 245, align: 'right' });
    ...
    for (let i = 0; i < range.count; i++) {
      ...
      doc.font('Helvetica').fontSize(8).fillColor(MUTED)
        .text('Generated by Vitarus Animal Diagnostics · Decision support only, not a substitute for veterinary diagnosis.', 50, 780, { width: 495, align: 'center' });
    }
